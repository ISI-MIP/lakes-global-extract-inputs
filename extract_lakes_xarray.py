...
for period in get_periods(path):
    ...
    for index, lake in lakes_df.iterrows():
        ...
        print('   read tasmax ...')
        tasmax_lake = tasmax_ds.sel(lat=lake[2], lon=lake[3], method='nearest', drop=True)
        print('   read hurs ...')
        hurs_lake = hurs_ds.sel(lat=lake[2], lon=lake[3], method='nearest', drop=True)
        print('   read huss ...')
        huss_lake = huss_ds.sel(lat=lake[2], lon=lake[3], method='nearest', drop=True)
        print('   read pr ...')
        pr_lake = pr_ds.sel(lat=lake[2], lon=lake[3], method='nearest', drop=True)
        if process_prsn:
            print('   read prsn ...')
            prsn_lake = prsn_ds.sel(lat=lake[2], lon=lake[3], method='nearest', drop=True)
        print('   read rsds ...')
